...
**A real bug this asymmetry caused, since fixed:** E4's deletion routine originally chained three `retrieve` lookups (`PartUsage`, then its `ActionUsage`/`RequirementUsage` siblings) in one `match` block. Since `retrieve` is a silent match-gate, not a nullable lookup, the whole match only ever succeeded when *all three* correspondences existed — which is never true for Rule G blocks, since the scope guard above means they never get Rule D siblings in the first place. The practical effect: deleting a `BusSelector`/`Goto`/etc. silently did nothing on the SysML side — the stale `PartUsage` was never cleaned up — for as long as Rule G has existed, since nothing ever tested deleting one of these block types until it was added here. Fixed by splitting into three independently-gated routines, mirroring the pattern P2's rename routine already used for the same reason (`syncBlockNameToPartUsage`/`syncCascadeFunctionName`/`syncCascadeRequirementName`).
